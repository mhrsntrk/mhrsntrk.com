...
export default function PostBody({ content }) {
  ...
  useEffect(() => {
    ...
    Array.from(container.querySelectorAll('code.language-mermaid')).forEach(
      (code) => {
  ...
  useEffect(() => {
    ...
    pres.forEach((preEl) => {
      ...
      btn.className =
        'absolute top-2 right-2 z-10 px-2 py-1 text-xs text-gray-700 border border-gray-300 rounded-md bg-white/80 backdrop-blur hover:bg-gray-100 active:bg-gray-200 dark:text-gray-100 dark:border-gray-700 dark:bg-gray-800/70 dark:hover:bg-gray-700';
