...
function validateInput(input, type = 'string') {
  ...
  switch (type) {
    ...
    case 'number':
      ...
      return isNaN(num)
        ? null
        : Math.max(0, Math.min(num, CONFIG.MAX_PAGE_SIZE));
    case 'slug':
      return typeof input === 'string'
        ? input.replace(/[^a-zA-Z0-9\-_]/g, '').slice(0, 100)
        : null;
    default:
      return input;
  }
...
export async function wakeUpStrapi(isBuildTime = false) {
  ...
  const timeout = isBuildTime
    ? CONFIG.BUILD_REQUEST_TIMEOUT
    : CONFIG.REQUEST_TIMEOUT;

  console.log(
    `[wakeUpStrapi] Attempting to wake up Strapi (build time: ${isBuildTime})`
  );
    ...
    try {
      ...
      try {
        const response = await fetch(
          `${STRAPI_BASE_URL}/api/posts?pagination[pageSize]=1`,
          {
        ...
        if (response.ok) {
          console.log(
            `[wakeUpStrapi] Strapi is awake after ${attempt} attempt(s)`
          );
          return true;
        }
        ...
        if (response.status < 500) {
          console.log(
            `[wakeUpStrapi] Strapi is responding (status: ${response.status})`
          );
          return true;
        }
      } catch (fetchError) {
        ...
        console.log(
          `[wakeUpStrapi] Attempt ${attempt}/${maxAttempts} failed, waiting ${delay}ms before retry...`
        );
      ...
      if (attempt === maxAttempts) {
        console.error(
          `[wakeUpStrapi] Failed to wake up Strapi after ${maxAttempts} attempts:`,
          error.message
        );
        throw new Error(
          `Strapi wake-up failed after ${maxAttempts} attempts: ${error.message}`
        );
      }
...
async function secureFetch(
  ...
  // Rate limiting guards against external abuse at runtime. It must NOT throttle
  // our own build: static generation fires 2 requests per post (getPostAndMorePosts)
  // plus paths/sitemap/artifacts, easily exceeding the 100/min budget, which made
  // getStaticProps swallow the error and silently 404 the tail of the post list.
  if (!isBuildTime) {
  ...
  const timeout = isBuildTime
    ? CONFIG.BUILD_REQUEST_TIMEOUT
    : CONFIG.REQUEST_TIMEOUT;
  const maxRetries = isBuildTime
    ? CONFIG.BUILD_MAX_RETRIES
    : CONFIG.MAX_RETRIES;
  const retryDelay = isBuildTime
    ? CONFIG.BUILD_RETRY_DELAY
    : CONFIG.RETRY_DELAY;
  ...
  try {
    ...
    if (
      ...
      console.log(
        `[secureFetch] First failure detected during build, attempting to wake up Strapi...`
      );
      try {
        ...
        console.error(
          `[secureFetch] Wake-up failed, continuing with retry logic:`,
          wakeError.message
        );
      }
    }
...
function buildQueryParams(params = {}) {
  ...
  if (params.page) {
    const page = validateInput(params.page, 'number');
    if (page) query.set('pagination[page]', page);
  }
  ...
  // Populate
  if (params.populate) {
    const populateFields = Array.isArray(params.populate)
      ? params.populate
      : [params.populate];
  ...
  return json.data
    .map((item) => {
      ...
      return {
        ...
        updatedAt: item.updatedAt || item.Date || item.publishedAt || null,
...
export async function getAllPostsWithSlug(isBuildTime = false) {
  try {
    const json = await secureApiRequest(
      '/api/posts',
      { pageSize: 1000 },
      isBuildTime
    );
...
export async function getAllPostsForBlog(isBuildTime = false) {
  try {
    // Strapi caps pageSize at CONFIG.MAX_PAGE_SIZE (100), so page through the
    // full result set. Consumers (sitemap, RSS, llms-full) need every post.
    const allPosts = [];
    let page = 1;
    let pageCount = 1;
    do {
      ...
      allPosts.push(...mapPostsFromResponse(json));
...
export async function getPostAndMorePosts(slug, isBuildTime = false) {
  ...
  try {
    ...
    console.error(
      `[getPostAndMorePosts] Failed to fetch post "${sanitizedSlug}":`,
      error.message
    );
