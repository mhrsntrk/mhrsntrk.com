...
import * as key from '@cef-ebsi/key-did-resolver';

// DID validation regex patterns
const DID_PATTERN = /^did:[a-z0-9]+:[a-zA-Z0-9._-]+$/;
const SUPPORTED_METHODS = ['web', 'ethr', 'key'];
...
function validateDID(did) {
  ...
  if (!DID_PATTERN.test(trimmedDid)) {
    return {
      valid: false,
      error: 'Invalid DID format. Expected format: did:method:identifier'
    };
  }
...
function analyzeDidDocument(didDocument) {
  ...
  if (actualDidDocument.verificationMethod) {
    ...
    analysis.verificationMethodCount = Array.isArray(
      actualDidDocument.verificationMethod
    )
  ...
  if (actualDidDocument.authentication) {
    ...
    analysis.authenticationCount = Array.isArray(
      actualDidDocument.authentication
    )
  ...
  if (actualDidDocument.assertionMethod) {
    ...
    analysis.assertionMethodCount = Array.isArray(
      actualDidDocument.assertionMethod
    )
  ...
  if (actualDidDocument.capabilityDelegation) {
    ...
    analysis.capabilityDelegationCount = Array.isArray(
      actualDidDocument.capabilityDelegation
    )
  ...
  if (actualDidDocument.capabilityInvocation) {
    ...
    analysis.capabilityInvocationCount = Array.isArray(
      actualDidDocument.capabilityInvocation
    )
