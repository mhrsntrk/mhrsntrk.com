...
async function resolveWithProvider(providerConfig, ensName, timeoutMs = 10000) {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        `Avatar lookup failed with ${providerConfig.name}:`,
        avatarError.message
      );
    }
...
async function resolveWithEtherscanV2(ensName) {
  try {
    ...
    const address = ethers.utils.getAddress(
      '0x' + addressResult.result.slice(26)
    );
    ...
    try {
      ...
      const avatarKey = ethers.utils.keccak256(
        ethers.utils.toUtf8Bytes('avatar')
      );
      const avatarData =
        textFunctionSelector + nameHash.slice(2) + avatarKey.slice(2);
      ...
      console.warn(
        'Avatar lookup via Etherscan V2 failed:',
        avatarError.message
      );
    }
