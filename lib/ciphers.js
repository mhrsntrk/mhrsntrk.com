...
export const CIPHERS = [
  ...
  },
  {
    ...
    year: '1553',
    blurb:
      'A different Caesar shift for every letter, cycling through a keyword. Giovan Battista Bellaso published it in 1553 and history handed the credit to Vigenere, who had published a different cipher entirely. Called le chiffre indechiffrable until Kasiski broke it in 1863.',
    param: 'key',
...
/**
 * Chi squared per letter, so the number does not simply grow with length.
 * Returns null for text too short to judge honestly.
 */
export function englishScore(text) {
  const { total } = letterCounts(text);
  if (total < 12) return null;
  return chiSquared(text) / total;
}

/**
 * There is deliberately no absolute "this is English" test here. Measured over
 * real sentences the ranges overlap: a pangram scores 3.12 while a wrong Caesar
 * shift of ordinary prose can score 1.87, so any fixed threshold would both
 * miss real solutions and announce false ones.
 *
 * What is reliable is the comparison. When every candidate key can be
 * enumerated, the best scoring one is almost always the answer, which is what
 * crackCaesar and crackRailFence below report.
 */
...
/**
 * Every rail count from 2 up, unranked and in order.
 *
 * Deliberately not scored. Rail fence is a transposition: it reorders letters
 * without substituting any of them, so every candidate has exactly the same
 * letter distribution and chi squared cannot tell them apart. Ranking these
 * measured 0 correct out of 12, always returning 2. Breaking it properly needs
 * bigram or quadgram statistics, which read letter order rather than letter
 * counts. With only seven options, showing them all and letting a human read is
 * both honest and faster.
 */
export function railFenceCandidates(ciphertext, maxRails = 8) {
  const out = [];
  for (let rails = 2; rails <= maxRails; rails += 1) {
    out.push({ rails, plain: railFence(ciphertext, rails, true) });
  }
  return out;
}

/**
 * The best guess at a key, for the ciphers whose keyspace is both small enough
 * to walk and vulnerable to letter counting. That is Caesar, and only Caesar.
 * Returns null for Vigenere and keyword substitution, where the key genuinely
 * has to come from the sender, and for rail fence, per the note above.
 */
export function bestGuess(cipherId, ciphertext) {
  const { total } = letterCounts(ciphertext);
  if (total < 12) return null;
  if (cipherId === 'caesar') {
    return { param: 'shift', value: crackCaesar(ciphertext)[0].shift };
  }
  return null;
}
