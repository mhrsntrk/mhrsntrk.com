...
export default async function handler(req, res) {
  ...
  if (!consent) {
    return res.status(400).json({ error: 'Please accept the privacy policy to subscribe.' });
  }
  ...
  try {
    ...
    const resp = await fetch(`${LISTMONK_URL.replace(/\/$/, '')}/api/subscribers`, {
      method: 'POST',
    ...
    if (resp.status === 409 || /already exists|already a subscriber/i.test(detail)) {
      return res.status(200).json({
        ok: true,
